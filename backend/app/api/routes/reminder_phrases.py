...
def create_reminder_phrase(*, session: SessionDep, current_user: CurrentUser, reminder_phrase_in: ReminderPhraseCreate) -> Any:
    if not current_user.is_superuser:
        logger.warning(f"Non-superuser attempted to create reminder phrase user_id={current_user.id}")
    ...
    reminder_phrase = ReminderPhrase.model_validate(reminder_phrase_in, update={"created_at": datetime.now(timezone.utc), "updated_at": datetime.now(timezone.utc)})
    ...
    try:
        ...
        if "reminder_phrases_middah_text_uq" in error_info or "unique" in error_info.lower():
            raise HTTPException(status_code=400, detail="Reminder phrase already exists for this middah and text")
...
def get_reminder_phrase(session: SessionDep, current_user: CurrentUser, id: int) -> Any:
    ...
    if not reminder_phrase:
        logger.warning(f"Reminder phrase not found user_id={current_user.id} reminder_phrase_id={id}")
...
def patch_reminder_phrase(*, session: SessionDep, current_user: CurrentUser, id: int, patch: ReminderPhrasePatch) -> Any:
    if not current_user.is_superuser:
        logger.warning(f"Non-superuser attempted to patch reminder phrase user_id={current_user.id} reminder_phrase_id={id}")
        raise HTTPException(status_code=403, detail="The user doesn't have enough privileges")
    ...
    logger.info(f"Patching reminder phrase user_id={current_user.id} reminder_phrase_id={id} {update_dict}")
    ...
    try:
        ...
        session.rollback()
        error_info = str(e.orig)
        logger.error(f"IntegrityError patching reminder phrase reminder_phrase_id={id} {error_info}")
        if "reminder_phrases_middah_text_uq" in error_info or "unique" in error_info.lower():
            raise HTTPException(status_code=400, detail="Reminder phrase already exists for this middah and text")
        elif "foreign key constraint" in error_info.lower():
            raise HTTPException(status_code=400, detail="Invalid middah specified")
...
def delete_reminder_phrase(*, session: SessionDep, current_user: CurrentUser, id: int):
    if not current_user.is_superuser:
        logger.warning(f"Non-superuser attempted to delete reminder phrase user_id={current_user.id} reminder_phrase_id={id}")
        raise HTTPException(status_code=403, detail="The user doesn't have enough privileges")
